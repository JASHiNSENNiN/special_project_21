...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !isset($_POST['change_password'])) {
    ...
    // Upload images
    $profile_image = uploadImage($_FILES['profile_image'] ?? null);
    $cover_image = uploadImage($_FILES['cover_image'] ?? null);

    // Handle update of school name
    if (isset($_POST['school_name'])) {
        // Sanitize inputs
        $school_name = filter_var($_POST['school_name'], FILTER_SANITIZE_STRING);

        // Update school profile
        $sql = "UPDATE school_profiles SET school_name = ? WHERE user_id = ?";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("si", $school_name, $user_id);
                                            ...
                                            <div class="card mb-4">
                                                ...
                                                <div class="card-body">
                                                    <div class="row gx-3 mb-3">
                                                        <div class="col-md-12">
                                                            <label class="small mb-1" for="inputSchoolName">School
                                                                Name</label>
                                                            <input class="form-control" id="inputSchoolName"
                                                                name="school_name" type="text"
                                                                value="<?php echo htmlspecialchars($profile_data['school_name'] ?? ''); ?>"
                                                                pattern="[A-Za-z\s]*"
