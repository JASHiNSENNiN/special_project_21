...
function removeApplication(workId) {
  ...
  xhr.open(
    "POST",
    "/SHS-Work-Immersion-Tracker/php/remove_application.php",
    true
  );
  xhr.setRequestHeader("Content-Type", "application/json");
...
function checkApplicationStatus(workId, applyButton) {
  ...
  xhr.open(
    "GET",
    "/SHS-Work-Immersion-Tracker/php/check_application_status.php?workId=" +
      workId,
    true
  );
  xhr.setRequestHeader("Content-Type", "application/json");
  ...
  xhr.onload = function () {
    if (xhr.status === 200) {
      ...
      if (response.success) {
        ...
        const orgRatingsElement = document.getElementById("org-ratings");
        ...
        if (response.partner_profile === null) {
          ...
          orgRatingsElement.textContent = response.partner_profile.stars || "";
        }
...
function setWorkPage() {
  ...
  if (current_work === null) {
    ...
    const workImmersionName = document.getElementById("work-immersion-name");
    const workImmersionStrand = document.getElementById(
      "work-immersion-strand"
    );
    const workImmersionStars = document.getElementById("work-immersion-stars");
